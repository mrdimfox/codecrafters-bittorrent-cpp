...
inline static void reset(
  uint32_t digest[], std::string& buffer, uint64_t& transforms
)
{
...
inline static void transform(
  uint32_t digest[], uint32_t block[BLOCK_INTS], uint64_t& transforms
)
{
...
inline auto sha1_hash_to_bytes(std::string_view hash) -> std::vector<uint8_t>
{
